...
export function EligibiliteSection({ eligibility }: EligibiliteSectionProps) {
  const isEligible = eligibility.result === "PASS"
  const isRejected = eligibility.result === "FAIL" || eligibility.result === "FAILED"
  const hasFailed = eligibility.failedCriteria.length > 0
  ...
  return (
    <Card className="border border-stone-200 dark:border-stone-800 shadow-xs">
      ...
      <CardContent className="pb-4 pt-3 space-y-3 text-xs">
        ...
        {/* ── Critères non validés (only shown when there are failures) ── */}
        {hasFailed && (
          <ul className="space-y-1.5">
            {eligibility.failedCriteria.map((fc, i) => (
              <li key={i} className="flex items-start gap-2">
                <HugeiconsIcon icon={AlertCircleIcon} size={14} className="text-rose-600 shrink-0 mt-0.5" />
                <span className="font-mono text-rose-900 dark:text-rose-200">
                  {capitalizeFirst(fc)}
                </span>
              </li>
            ))}
                  ...
                  <div className="flex items-start gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      <span className="font-medium text-stone-900 dark:text-stone-100 block">
                        {capitalizeFirst(chk.criterionLabel)}
                      </span>
                      {chk.detail && (
